...
if __name__ == "__main__":
    ...
    #     try:
    #         print(
    #             f"Participant {participant}'s stats file is saved at {output_file}")
    ...
    #     os.path.dirname(sys.argv[1])), "google_map_data_stats.json")
    ...
    def update_histogram(selected_year):
        if selected_year and selected_year != 'All':
            data = [unique_places_visited_by_month[month]
                    for month in unique_places_visited_by_month if month.startswith(selected_year)]
        else:
